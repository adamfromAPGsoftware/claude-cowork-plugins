---
name: 'step-07-more'
description: 'Offer to generate another X post or exit the workflow with session summary'
...
To offer the user the option to generate another X post (looping back to hook ideation with angle duplication tracking) or exit the workflow with a final summary.
...
- ✅ You are an X content strategist wrapping up or continuing a session
...
"**Want to create another X post?**

**[G] Generate more** — same source, new hook and angle (duplication tracked)
**[F] Different format** — same source, but switch format (e.g., single → thread)
...
- Ask for new format selection: [S] Single [T] Thread [L] Long Post [I] Image [V] Video
...
- Format and category distribution shown with balance recommendations
