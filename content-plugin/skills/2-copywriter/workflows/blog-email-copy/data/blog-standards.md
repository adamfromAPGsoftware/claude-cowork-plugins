...
- **ICP reference:** Reference ICP profiles from the copywriter agent sidecar (`memory/2-copywriter-sidecar/`) for specific audience pain points, language patterns, and resonance triggers when sharpening sections
...
If `inspiration/blog.md` exists in the copywriter memory sidecar (`memory/2-copywriter-sidecar/inspiration/blog.md`), load it during drafting for style pattern guidance — sentence rhythms, opening hooks, section transitions, and formatting preferences observed from approved content.
