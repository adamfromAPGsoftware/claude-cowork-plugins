# Instagram Carousel — Prompt Guidelines ({YOUR_COMPANY} / Dark Mode)

Reference for crafting fal-ai/nano-banana-2 image-generation prompts for **@{YOUR_HANDLE}** Instagram carousel slides. Every prompt MUST include the brand anchors. The visual identity is professional, polished, and tech-forward — DM Sans typography, neon lime (#90F23C) accents on dark backgrounds. Targeting SME founders and decision-makers with ROI-focused, "we built this" energy.
...
| Handle | `@{YOUR_HANDLE}` bottom-left on content slides |
| Bookmark | Bookmark icon + "save for later" bottom-right on content slides |
| Palette | Dark-dominant — pure black, near-black surfaces (#111111), neon lime accents |
| Logo | Attached logo image displayed top-left at ~40px on every slide |
...
Bottom-left: "@{YOUR_HANDLE}" in small white text.
Bottom-right: bookmark icon with "save for later" in small text.
```
...
arrow/chevron for emphasis]. Bottom-left: "@{YOUR_HANDLE}" in small white text.
Bottom-right: bookmark icon with "save for later" in small text.
```
...
branding only. Bottom-left: "@{YOUR_HANDLE}" in small white text.
Bottom-right: bookmark icon with "save for later" in small text.
```
...
### Personality & Warmth
- Keep the conversational, approachable energy in all slides
- Creator photo slides should still feel candid and real (not corporate headshot energy)
- Body text tone stays casual and direct — DM Sans polishes it without losing warmth
...
2. **Content slides** (3-6) — each makes ONE point. Vary layouts between slides. Include @{YOUR_HANDLE} and bookmark.
...
- **Tone:** Conversational, lowercase-friendly, authentic
- **Hashtags:** 3-5 targeted, niche-specific hashtags per post. Treat them like SEO keywords.
  ...
  - {YOUR_COMPANY} focus: business automation, ROI, enterprise AI (e.g. `#aiagency` `#saasreplacement` `#businessautomation`)
- **Structure:**
...
### Logo (MANDATORY on every slide)

The {YOUR_COMPANY} logo (green gear icon) MUST appear on every slide in the dark-mode carousel. Include it in every slide's `embed_images` array. The prompt for each slide must include: "Display the attached logo (green gear icon) in the top-left corner at roughly 40px". This is separate from tool/brand logos — the logo is the company watermark.
...
The CTA slide (last slide) is fully generated by nano-banana-2 from a text prompt — no photo of the creator. It's a bold "Comment [KEYWORD]" design with brand styling. See the CTA Slide prompt pattern in Section 2.
