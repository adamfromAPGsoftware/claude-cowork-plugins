---
name: cowork-full-cycle
description: Complete poll-and-respond cycle for all Instagram accounts. Designed for scheduled Cowork execution via Desktop Commander.
menu-code: FC
---

# Full Cycle: Poll and Respond (All Accounts)

This is a self-contained, zero-context workflow for scheduled Cowork execution. It polls all Instagram accounts for new DMs and comments, then generates and sends responses using each account's conversation strategy. Everything runs through Desktop Commander.
...
```
execute_command("ls {PROJECT_ROOT}/social-plugin/accounts/")
```

This returns a list of account folder names (e.g., `example-account`). Each folder is an account key. Process ALL of them.

---

## Step 2: For Each Account

Repeat the entire sequence below for every account discovered in Step 1.

### 2a. Read Account Config

```
execute_command("cat {PROJECT_ROOT}/social-plugin/accounts/{account_key}/config.json")
```
...
```
execute_command("cd {PROJECT_ROOT} && python3 social-plugin/scripts/fetch-instagram-dms.py --account {account_key}")
```
...
```
execute_command("cd {PROJECT_ROOT} && python3 social-plugin/scripts/fetch-instagram-comments.py --account {account_key} --days 3")
```
...
```
execute_command("cat {PROJECT_ROOT}/social-plugin/data/{account_key}/social-data.json")
```
...
```
execute_command("cat {PROJECT_ROOT}/social-plugin/accounts/{account_key}/conversation-strategy.md")
```
```
execute_command("cat {PROJECT_ROOT}/social-plugin/accounts/{account_key}/brand-voice.md")
```
```
execute_command("cat {PROJECT_ROOT}/social-plugin/accounts/{account_key}/products.md")
```
...
```
execute_command("cd {PROJECT_ROOT} && python3 social-plugin/scripts/send-instagram-dm.py --account {account_key} --conversation-id {conv_id} --message '{response}'")
```
...
```
execute_command("cd {PROJECT_ROOT} && python3 social-plugin/scripts/send-instagram-comment-reply.py --account {account_key} --comment-id {comment_id} --message '{response}'")
```
...
```
execute_command("cat > {PROJECT_ROOT}/social-plugin/data/{account_key}/social-data.json << 'ENDOFFILE'
{updated_json}
ENDOFFILE")
```
...
After processing ALL accounts, output a structured summary:
